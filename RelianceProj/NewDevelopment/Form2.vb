...
Public Class Form2
    ...
    Private Sub MenuItem_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'Dim n As Integer = Screen_Width - GridControl1.Width
        'GridControl1.Height = Me.Height - 45
        'GridControl1.Location = New Point(n, Me.Location.Y + 35)
        ...
        End With
        RS = _Query.ToString
        'Dim query As String = "SELECT MenuID, ParentMenuID, MenuName, Level1,IsSepara FROM MenuItems ORDER BY ParentMenuID, MenuID"
        _newconnectionOpen()
        Dim command As New OleDbCommand(RS, NewDbConnection)

        ' Execute the query
        Using reader As OleDbDataReader = command.ExecuteReader()
            ' Dictionary to hold menu items by their MenuID
            ...
            While reader.Read()
                ...
                If isSeparator Then
                    ' Create a separator and add it to the parent menu
                ...
                Else
                    ...
                    AddHandler newMenuItem.Click, AddressOf MenuItem_Click ' Assign click event
                    ...
                    If parentMenuID <> 0 Then
                        ...
                        End If
                    Else
                        ' If no parent, it's a top-level menu, so add it to the MenuStrip
    ...
    Private Sub MenuItem_Click(sender As Object, e As EventArgs)
        ...
        If mnuItem.Tag <> "" Then
            ...
            'Dim Work_Width As Integer = gMDI.Width - pnl_Help_Btn.Width
            'Dim Work_Height As Integer = gMDI.Height - (lbl_Footer_Main.Height + mnuAgency.Height + mnuAgency.Height + 10)

            'If Work_Width < frm.Width Then
            '    pnl_Help_Btn.Hide()
            '    lbl_Footer_Main.Hide()
            '    lbl_Footer_Sub.Hide()
            '    lbl_Date.Hide()
            'End If

            'If Work_Height < frm.Height Then
            '    pnl_Help_Btn.Hide()
            '    lbl_Footer_Main.Hide()
            '    lbl_Footer_Sub.Hide()
            '    lbl_Date.Hide()
            ...
            If Frm_Name_For_Active.ToString = "QuitWithBackup" Then
                ...
                'Dim Cur_Date As String = Mid(ObjCls_General.GetTodayDate_British, 1, 2) + "-" + Mid(ObjCls_General.GetTodayDate_British, 4, 2) + "-" + Mid(ObjCls_General.GetTodayDate_British, 7, 4)
                ...
                'Dim DatabaseName As String = Start_Frm.Label8.Text
                'Wait_Window_Show(Me, "Backup System Runing Wait.....")
                ...
                'Dim _ADP As New OleDbDataAdapter(MSA_CMD)
                'Dim tbl As New DataTable
                '_ADP.Fill(tbl)
                ...
                For Each dr As DataRow In tbl.Select()
                    ...
                    'sqL = " backup database " & (dr("Data_Folder_Name") & "") & " to disk='" & Backup_Directory & "\" & (dr("Data_Folder_Name") & "") & ".bak" & "'   With DIFFERENTIAL   "
                    sql_connect_slect()
    ...
    Private Sub GridView2_Click(sender As Object, e As EventArgs) Handles GridView2.DoubleClick
