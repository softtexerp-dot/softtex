...
#Region "GRID COL. DEFINE AND FORMATTING WEAVING AND GREYCOST"
    Private Sub defineGridColNameWeaving()
        ...
        With _WeavingFieldHeader
            .Append("yarn_for:Weaving Sundary,")
            .Append("COUNTCODE:Add/Less,")
            .Append("FD_PD:Calc. By,")
            .Append("Reed:Per/Amt,")
            .Append("Yarn_Amount:Amount")
    ...
    End Sub
#End Region
#Region "GRID COL. DEFINE AND FORMATTING FINISH COST"
    ...
    Private Sub defineGridColNameFinish()
        ...
        With _FINISHFieldHeader
            .Append("yarn_for:Finish Sundary,")
            .Append("COUNTCODE:Add/Less,")
            .Append("FD_PD:Calc. By,")
            .Append("Reed:Per/Amt,")
            .Append("Yarn_Amount:Amount")
...
#Region "Form Default values on Load Yarn"
    Private Sub DefineDafaultValuesYarn()
        ...
        With _Fieldqry
            ...
            .Append(" FROM Query1 where 1=1 and BookName='YARN SUNDARY' and Y_JOB_WORKER_STK_OWN='Cost Sheet Setting' and Y_JOB_PARTY_STK_FLD='YES' order by Bookorder")
        End With
...
#End Region
#Region "Form Default values on Load WEAVING AND GREY COST"
    Private Sub DefineDafaultValuesWeaving()
        ...
        With _Fieldqry
            ...
            .Append(",'' as sales_cost")
            .Append(", 0.00 AS Amount")
            .Append(" FROM Query1 where 1=1 and BookName='WEAVING AND GREY COST' and Y_JOB_WORKER_STK_OWN='Cost Sheet Setting' and Y_JOB_PARTY_STK_FLD='YES' order by Bookorder")
        End With
        strQuery = _Fieldqry.ToString()
        sqL = strQuery
        sql_connect_slect()
...
#Region "Form Default values on Load FINISH COST"
    Private Sub DefineDafaultValuesFinishcost()
        ...
        With _Fieldqry
            ...
            .Append(",'' as sales_cost")
            .Append(", 0.00 AS Amount")
            .Append(" FROM Query1 where 1=1 and BookName='FINISH COST' and Y_JOB_WORKER_STK_OWN='Cost Sheet Setting' and Y_JOB_PARTY_STK_FLD='YES' order by Bookorder")
        End With
        strQuery = _Fieldqry.ToString()
        sqL = strQuery
        sql_connect_slect()
...
#Region "Form Default values on Load SALES COST"
    Private Sub DefineDafaultValuesSalescost()
        ...
        With _Fieldqry
            ...
            .Append(",'' as sales_cost")
            .Append(", 0.00 AS Amount")
            .Append(" FROM Query1 where 1=1 and BookName='SALES COST' and Y_JOB_WORKER_STK_OWN='Cost Sheet Setting' and Y_JOB_PARTY_STK_FLD='YES' order by Bookorder")
        End With
        strQuery = _Fieldqry.ToString()
        sqL = strQuery
        sql_connect_slect()
...
#Region "ALTER FORM QUERY "
    Private Function getAlter_Form_Query_Details(ByVal strKeyID As String) As String
        ...
        With _strQuery
            ...
            .Append(" WHERE 1=1  ")
            .Append(" AND A.ENTRYNO=" & Val(strKeyID) & " AND UPPER(ISNULL(A.OP1,'')) <> 'YARN INFORMATION' ")
...
#Region "ALTER FORM QUERY Yarn"
    Private Function getAlter_YarnForm_Query_Details(ByVal strKeyID As String) As String
        ...
        With _strQuery
            ...
            .Append(" WHERE 1=1  ")
            .Append(" AND ENTRYNO=" & Val(strKeyID) & " AND UPPER(ISNULL(OP1,''))= 'YARN INFORMATION' and Fabric_Item_Name='YARN SUNDARY' ")
...
#Region "ALTER FORM QUERY Weaving"
    Private Function getAlter_WeavingForm_Query_Details(ByVal strKeyID As String) As String
        ...
        With _strQuery
            ...
            .Append(" FROM  TRNFABRICCOST")
            .Append(" WHERE 1=1  ")
            .Append(" AND ENTRYNO=" & Val(strKeyID) & " AND UPPER(ISNULL(OP1,''))= 'YARN INFORMATION' and Fabric_Item_Name='WEAVING AND GREY COST' ")
            .Append(" ORDER BY OTHEREXP_1 ")
        End With
        Return _strQuery.ToString
    End Function
...
#Region "ALTER FORM QUERY FINISH COST"
    Private Function getAlter_FinishForm_Query_Details(ByVal strKeyID As String) As String
        ...
        With _strQuery
            ...
            .Append(" FROM  TRNFABRICCOST")
            .Append(" WHERE 1=1  ")
            .Append(" AND ENTRYNO=" & Val(strKeyID) & " AND UPPER(ISNULL(OP1,''))= 'YARN INFORMATION' and Fabric_Item_Name='FINISH COST' ")
            .Append(" ORDER BY OTHEREXP_1 ")
        End With
        Return _strQuery.ToString
    End Function
...
#Region "ALL RATES CALC SYSTEM "
    ...
    Private Sub Rate_Calc()
        ...
        For i As Int16 = 1 To GrdWeavingcost.Rows - 1
            Dim reedValue As Decimal = 0D
            Decimal.TryParse(GrdWeavingcost.Cell(i, _WeavingDataTableGrid.Columns.IndexOf("Reed") + 1).Text, reedValue)
            Dim weavingAmount As Decimal = 0
            'BEHAVIOUR
            If GrdWeavingcost.Cell(i, _WeavingDataTableGrid.Columns.IndexOf("Fabric_Design_No") + 1).Text = "0000-000000005" Then
                'weavingAmount = Math.Round((reedValue * txt_pick.Text), 2)
                Dim pickValue As Decimal = 0D
                If Decimal.TryParse(txt_pick.Text.Trim(), pickValue) And GrdWeavingcost.Cell(i, _WeavingDataTableGrid.Columns.IndexOf("FD_PD") + 1).Text = "AMOUNT" Then
                    weavingAmount = Math.Round((reedValue), 2)
                ElseIf Decimal.TryParse(txt_pick.Text.Trim(), pickValue) Then
                    weavingAmount = Math.Round(reedValue * pickValue, 2)
                Else
                    weavingAmount = 0D   ' ya message show karein
                End If
            ElseIf GrdWeavingcost.Cell(i, _WeavingDataTableGrid.Columns.IndexOf("FD_PD") + 1).Text = "AMOUNT" Then
                'weavingAmount = Math.Round((reedValue + weavingsubTotal), 2)
                weavingAmount = Math.Round((reedValue), 2)
            Else
                weavingAmount = Math.Round((reedValue * weavingsubTotal) / 100D, 2)
                'weavingAmount = Math.Round((reedValue * weavingsubTotal), 2)
            ...
            If GrdWeavingcost.Cell(i, _WeavingDataTableGrid.Columns.IndexOf("COUNTCODE") + 1).Text = "LESS" Then
                weavingAmount = "-" & weavingAmount
            End If
            If GrdWeavingcost.Cell(i, _WeavingDataTableGrid.Columns.IndexOf("COUNTCODE") + 1).Text = "NON" Then
                weavingAmount = 0
            End If
            Dim colIndex As Integer = _WeavingDataTableGrid.Columns.IndexOf("Yarn_Amount") + 1
            GrdWeavingcost.Cell(i, colIndex).Text = weavingAmount.ToString("0.00")
            weavingsubTotal = weavingsubTotal + weavingAmount
        ...
        For i As Int16 = 1 To GrdFinishcost.Rows - 1
            Dim reedValue As Decimal = 0D
            Decimal.TryParse(GrdFinishcost.Cell(i, _FINISHDataTableGrid.Columns.IndexOf("Reed") + 1).Text, reedValue)
...
#End Region
    ...
    Private Sub TXT_Net_Sales_Cost_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TXT_Net_Sales_Cost.KeyDown, MyBase.KeyDown
        If e.KeyCode = 13 Then
...
#Region "GRID ITEM EVENTS FOR YARN"
    Private Sub Grdyarn_EnterRow(Sender As Object, e As FlexCell.Grid.EnterRowEventArgs) Handles Grdyarn.EnterRow
        If _FrmLoad = True Then Exit Sub
        _FrmLoad = True
        ...
        _FrmLoad = False
    End Sub
    Private Sub Grdyarn_Click(Sender As Object, e As EventArgs) Handles Grdyarn.Click
        _ActivatedColName = Trim(UCase(Sender.Cell(0, Sender.ActiveCell.Col).TAG))
        _FrmLoad = False
    End Sub
    Private Sub Grdyarn_RowColChange(Sender As Object, e As FlexCell.Grid.RowColChangeEventArgs) Handles Grdyarn.RowColChange
        If _FrmLoad = True Then Exit Sub
        _RowNo = e.Row
        _ColNo = e.Col
        _ActivatedColName = Trim(UCase(Sender.Cell(0, Sender.ActiveCell.Col).TAG))
        Grdyarn.ActiveCell.BackColor = Color.Transparent
    End Sub
    Private Sub Grdyarn_LeaveCell(Sender As Object, e As FlexCell.Grid.LeaveCellEventArgs) Handles Grdyarn.LeaveCell
        If _FrmLoad = True Then Exit Sub
        If _AllowMoveFromCell = False Then e.Cancel = True
        Grdyarn.ActiveCell.BackColor = Grdyarn.BackColor1
    End Sub
    Private Sub Grdyarn_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Grdyarn.GotFocus
        _ActivatedColName = UCase(sender.Cell(0, sender.ActiveCell.Col).Tag)
        Grdyarn.ActiveCell.BackColor = Color.Transparent
        _FrmLoad = False
    End Sub
    Private Sub Grdyarn_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Grdyarn.LostFocus
        If _FrmLoad = True Then Exit Sub
        _LastRow = sender.ActiveCell.Row
    End Sub
    Private Sub Grdyarn_Validated(sender As Object, e As EventArgs) Handles Grdyarn.Validated
        If _FrmLoad = True Then Exit Sub
        Grdyarn.Refresh()
    End Sub
    Private Sub Grdyarn_LeaveRow(Sender As Object, e As FlexCell.Grid.LeaveRowEventArgs) Handles Grdyarn.LeaveRow
        If _FrmLoad = True Then Exit Sub
        _LastRow = Sender.ActiveCell.Row
    End Sub
    Private Sub Grdyarn_KeyDown(Sender As Object, e As KeyEventArgs) Handles Grdyarn.KeyDown
        If _FrmLoad = True Then Exit Sub
    ...
    Private Sub TXT_Net_Sales_Cost_OnVaidationError(_ErrorMsg As String) Handles TXT_Net_Sales_Cost.OnVaidationError
...
#Region "GRID ITEM EVENTS FOR SALES COST"
    Private Sub Grdsalescost_EnterRow(Sender As Object, e As FlexCell.Grid.EnterRowEventArgs) Handles Grdsalescost.EnterRow
        If _FrmLoad = True Then Exit Sub
        _FrmLoad = True
        ...
        _FrmLoad = False
    End Sub
    Private Sub Grdsalescost_Click(Sender As Object, e As EventArgs) Handles Grdsalescost.Click
        _ActivatedColName = Trim(UCase(Sender.Cell(0, Sender.ActiveCell.Col).TAG))
        _FrmLoad = False
    End Sub
    Private Sub Grdsalescost_RowColChange(Sender As Object, e As FlexCell.Grid.RowColChangeEventArgs) Handles Grdsalescost.RowColChange
        If _FrmLoad = True Then Exit Sub
        _RowNo = e.Row
        _ColNo = e.Col
        _ActivatedColName = Trim(UCase(Sender.Cell(0, Sender.ActiveCell.Col).TAG))
        Grdsalescost.ActiveCell.BackColor = Color.Transparent
    End Sub
    Private Sub Grdsalescost_LeaveCell(Sender As Object, e As FlexCell.Grid.LeaveCellEventArgs) Handles Grdsalescost.LeaveCell
        If _FrmLoad = True Then Exit Sub
        If _AllowMoveFromCell = False Then e.Cancel = True
        Grdsalescost.ActiveCell.BackColor = Grdsalescost.BackColor1
    End Sub
    Private Sub Grdsalescost_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Grdsalescost.GotFocus
        _ActivatedColName = UCase(sender.Cell(0, sender.ActiveCell.Col).Tag)
        Grdsalescost.ActiveCell.BackColor = Color.Transparent
        _FrmLoad = False
    End Sub
    Private Sub Grdsalescost_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Grdsalescost.LostFocus
        If _FrmLoad = True Then Exit Sub
        _LastRow = sender.ActiveCell.Row
    End Sub
    Private Sub Grdsalescost_Validated(sender As Object, e As EventArgs) Handles Grdsalescost.Validated
        If _FrmLoad = True Then Exit Sub
        Grdsalescost.Refresh()
    End Sub
    Private Sub Grdsalescost_LeaveRow(Sender As Object, e As FlexCell.Grid.LeaveRowEventArgs) Handles Grdsalescost.LeaveRow
        If _FrmLoad = True Then Exit Sub
        _LastRow = Sender.ActiveCell.Row
    End Sub
    Private Sub Grdsalescost_KeyDown(Sender As Object, e As KeyEventArgs) Handles Grdsalescost.KeyDown
        If _FrmLoad = True Then Exit Sub
